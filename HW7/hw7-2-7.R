...
### Q2: higher sharpe ratio (0.9467681 vs. 0.7361721)
#       -> tangent pf for 5 assets: return = 0.189, sd = 0.1710823
#       -> tangent pf for 2 assets: return = 0.2322442, sd = 0.2787657
#       -> Weights (same as the tangent point)
...
#            0.259      0.741
### Q3: 
#       Weights for risky assets 1.092085, risk-free asset -0.092085
...
an_rf <- mean(rf$RF/100)*252 # business days
...
legend("topleft", legend = c("Efficient frontier", 'CAL of full set of stocks', 'CAL of INTL-MSFT'), lty = 1, col = c("red", 'blue', 'green'))
...
best_E = pf_r[pos]
points(min_sigma, best_E, pch = 20, col = 'red')
...
max(sharpe)
pf_r[which(sharpe == max(sharpe)), ]
pf_sd[which(sharpe == max(sharpe)), ]
...
text(pf_sd[pos], pf_r[pos], labels = 'tangent point', pos = 2, col = 'red')

pf_r[which(sharpe == max(sharpe)), ]
pf_sd[which(sharpe == max(sharpe)), ]
pf_weight[which(sharpe == max(sharpe)), ]

#util_2 = pf_weight*(pf_r - an_rf) + an_rf - 5/2*(pf_weight^2)*(pf_sd^2)
# 1st derivative of utility
# pf_r - an_rf - 5*pf_weight*pf_sd = 0                                                     
risky_weight <- (pf_r[pos] - an_rf) / (pf_sd[pos]^2 * 5)
risky_weight
...
points(min_sigma, best_E, pch = 20)
...
max(sharpe_2)
tangent_r <- E[which(sharpe_2 == max(sharpe_2))]
tangent_sd <- sigma[which(sharpe_2 == max(sharpe_2))]
w[which(sharpe_2 == max(sharpe_2))]

points(tangent_sd, tangent_r, pch = 17, col = 'red')
text(tangent_sd, tangent_r, labels = 'tangent point', pos = 2, col = 'red')
